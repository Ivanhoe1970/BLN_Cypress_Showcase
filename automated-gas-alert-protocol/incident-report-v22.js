...
class IncidentReportGenerator {
    ...
    _validateForReport() {
      ...
      if (!(hasCoords || hasCivic)) missing.push('dispatch.location (lat/lng OR address/LSD)');
    ...
    generateWordDocument() {
      try {
        ...
        link.download = this._makeFilename({
            date: this._formatDateForFilenameTZ(currAlert.timestamp || new Date(), this.timezone),
    ...
    _extractAlertInformation(currAlert) {
        return {
          ...
          timeOfAlert: this._formatTimeOnlyInTZ(currAlert.timestamp || new Date(), this.timezone) + ' ' + this._getTzAbbrev(currAlert.timestamp || new Date().toISOString(), this.timezone),
      ...
      _cleanAlertType(alertTitle) {
        ...
        const cleaned = alertTitle.replace(/🚨\s*/, '').replace('Alert Management - ', '');
      ...
      _extractOrganizationDetails(currAlert) {
        return {
          organizationName: this._extractFromDOM('#employee-details')?.split(' - ')[1] || 'WM - Corporate',
          deviceName: this._extractFromDOM('#device-details')?.split('-')[0] || 'G7x-3571031421',
          deviceId: this._extractFromDOM('#device-details')?.match(/\d{10}/)?.[0] || '3571031421',
          userActualName: this._extractFromDOM('#employee-details')?.split(' - ')[0] || 'Zach Sowell (ID: 373595)'
        };
      }
    ...
    _extractDispatchDetails() {
      ...
      return {
        ...
        latLong: (location.lat!=null && location.lng!=null) ? `${location.lat}, ${location.lng}` : 'N/A',
        lsd: location.lsd || 'N/A',
        country: this._extractCountryFromAddress(location.address) || 'United States',
        provinceState: this._extractStateFromAddress(location.address) || 'Unknown',
        city: this._extractCityFromAddress(location.address) || 'Unknown',
        streetAddress: this._extractStreetFromAddress(location.address) || 'Unknown',
        additional: location.additional || 'N/A'
      };
    }
  
    _extractAlertSummary(currAlert) {
        const ackTime = currAlert.acknowledgedAt || window.protocolState?.acknowledgedAt;
        const dispatchTime =
    ...
    _extractDetailedTimeline() {
      ...
      entries.forEach(entry => {
        ...
        if (timeElement && contentElement) {
          ...
          const fullTimestamp = this._convertLogTimeToFullTimestamp(timeText, currAlert.timestamp);
          
          if (fullTimestamp && content && !this._isSystemEvent(content)) {
            timeline.push({
              ts: fullTimestamp,
              time: this._formatTimestampForReport(fullTimestamp),
              note: `<span class="note-italic">${this._escapeHtml(content)}</span>`
            });
          }
        }
      });
    ...
    _extractTimelineEvents() {
      ...
      if (currAlert.triggeredAt) {
        events.push({ timestamp: currAlert.triggeredAt, note: 'Alert triggered by device', type: 'system' });
      }
      if (currAlert.receivedAt) {
        events.push({ timestamp: currAlert.receivedAt, note: 'Alert received by server', type: 'system' });
      }
      if (currAlert.acknowledgedAt) {
        events.push({ timestamp: currAlert.acknowledgedAt, note: 'Alert acknowledged', type: 'system' });
      }
      ...
      if (resolutionTime) {
        events.push({ timestamp: resolutionTime, note: 'Alert resolved.', type: 'system' });
      }
    ...
    _extractTimestampFromLogEntry(entry, baseIso) {
      const m = entry.querySelector('.log-timestamp')?.textContent?.match(/(\d{2}):(\d{2}):(\d{2})/);
      if (!m) return null;
    ...
    _computeTimeToDispatch(ackIso, reqIso) {
      ...
      _makeFilename({date, deviceName, org, deviceId, alertType, resolution}) {
        // Extract user name from the current data
        const userName = this._extractFromDOM('#employee-details')?.split(' - ')[0]?.split(' (')[0] || 'Unknown User';
        
        const base = `Incident Report ${date} - ${userName} - ${org} - Device ID ${deviceId} - ${alertType} - ${resolution}`;
        return base.replace(/[\\/:*?"<>|]/g, '_') + '.doc';
      }
    ...
    _getReadableAlertType(alertType) {
      ...
      if (/\s/.test(alertType) || /H₂S|CO|O₂|LEL/i.test(alertType)) return alertType;
    ...
    _findResolutionTime() {
      ...
      for (const entry of log.querySelectorAll('.log-entry')) {
        if (/alert resolved|resolution|resolving alert/i.test(entry.textContent || '')) {
    ...
    _extractCountryFromAddress(address) {
      ...
      if (s.includes('canada') || /\b(ab|bc|on|qc|mb|sk|nl|nb|pe|nt|nu|yt)\b/.test(s)) {
        return 'Canada';
      }
    ...
    _extractStateFromAddress(address) {
      ...
      const stateMatch = address.match(/\b([A-Z]{2})\b(?=\s*\d{5}|\s*[A-Z]\d[A-Z]\s*\d[A-Z]\d|\s*$)/);
    ...
    _generateWordHTML(data) {
      ...
      <head>
        ...
        <style>
          ...
          body { font-family: Arial, sans-serif; font-size: 9pt; line-height: 1.1; margin: 0; }
          .logo { color: #666; font-size: 12pt; margin-bottom: 5pt; }
          .logo .safety { color: #d32f2f; }
          .header { text-align: right; color: #666; font-style: italic; margin-bottom: 15pt; font-size: 12pt; }
          ...
          td { border: none; padding: 4pt; vertical-align: top; font-size: 9pt; }
      ...
      <h2 id="alert-information">Alert Information</h2>
      <table>
        <tr>
      ...
      <h2 id="organization-details">Organization Details</h2>
      <table>
        <tr>
      ...
      <table>
        <tr>
          <td colspan="2"><span class="field-label">Time from Alert Acknowledgement to Dispatch:</span> <em>${data.alertSummary.timeFromAckToDispatch}</em></td>
        </tr>
      </table>
      
      <div class="challenges-label">Challenges/Notes:</div>
      <div style="margin-bottom: 10pt;">${this._escapeHtml(data.alertSummary.challengesNotes)}</div>
      ...
      <table class="timeline-table">
        ...
        const timelineRows = data.alertSummary.detailedTimeline.map(entry => `
        <tr>
        ...
        return header + alertInfoSection + orgDetailsSection + dispatchDetailsSection + alertSummarySection + timelineRows + footer;
      }
    ...
    window.resolveAlert = function(...args) {
      ...
      setTimeout(() => {
        ...
        if (generator && generator.detectDispatchStatus()) {
          console.log('Alert resolved with dispatch - auto-generating incident report');
