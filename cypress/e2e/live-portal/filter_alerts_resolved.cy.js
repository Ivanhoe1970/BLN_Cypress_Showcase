...
describe('Filter Alerts by Resolved Status', () => {
  ...
  it('should filter alerts by Resolved status', () => {
    cy.log('🎯 Starting Resolved filter test');
    
    // Take screenshot for debugging
    cy.screenshot('before-resolved-filter');
    ...
    cy.get('body').then(($body) => {
      ...
      if (hasFilter) {
        try {
          ...
          cy.screenshot('after-resolved-filter-applied');
          ...
          cy.get('body').then(($b) => {
            ...
            if (rows.length > 0) {
              ...
              cy.log('ℹ️ No results found for Resolved status');
              AlertsPage.validateNoResults();
            }
          });
        } catch (error) {
          cy.log(`⚠️ Filter operation failed: ${error.message}`);
          cy.screenshot('resolved-filter-error');
