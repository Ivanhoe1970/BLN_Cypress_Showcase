...
describe('Filter Alerts by Acknowledged Status', () => {
  ...
  it('should filter alerts by Acknowledged status', () => {
    cy.log('🎯 Starting Acknowledged filter test');
    
    // Take screenshot for debugging
    cy.screenshot('before-acknowledged-filter');
    ...
    cy.get('body').then(($body) => {
      ...
      if (hasFilter) {
        try {
          ...
          cy.screenshot('after-acknowledged-filter-applied');
          ...
          cy.get('body').then(($b) => {
            ...
            if (rows.length > 0) {
              ...
              cy.log('ℹ️ No results found for Acknowledged status');
              AlertsPage.validateNoResults();
            }
          });
        } catch (error) {
          cy.log(`⚠️ Filter operation failed: ${error.message}`);
          cy.screenshot('acknowledged-filter-error');
